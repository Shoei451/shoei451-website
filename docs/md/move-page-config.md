# move-page-config — PAGE_CONFIG を list.js に移動するスクリプト

**配置先:** `scripts/move-page-config.mjs`

---

## 概要

`index.html` 内にインラインで書かれた `window.PAGE_CONFIG = {...}` ブロックを抽出し、`list.js` の末尾に移動する。その後 `index.html` をスリム版（slug リダイレクトなし・`sub-index.html` と同じ構造）に置き換える。
...
```bash
# dry-run（確認のみ）
node scripts/move-page-config.mjs --dry-run

# フォルダ指定
node scripts/move-page-config.mjs --only koten

# 全自動検出して実行
node scripts/move-page-config.mjs
```
...
- `index.html` に `window.PAGE_CONFIG` が含まれている
- `list.js` が存在する
- `list.js` に `PAGE_CONFIG` がまだない（移動済みをスキップ）
...
function collectCandidates(dir) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (html.includes('window.PAGE_CONFIG') && !listJs.includes('PAGE_CONFIG')) {
      results.push(folder);
    }
  }
  return results;
...
for (const folder of targets) {
  ...
  if (!configBlock) {
    console.warn(`  ⚠ ${rel}/index.html から PAGE_CONFIG を抽出できませんでした（スキップ）`);
    skipped++;
    continue;
  }

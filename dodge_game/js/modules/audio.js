  ...
  try {
    ...
    switch (type) {
      ...
      case 'hit': {
        ...
        dist.curve = curve;
        crack.connect(dist); dist.connect(crackGain); crackGain.connect(ac.destination);
        crack.type = 'sawtooth';
      ...
      case 'boss_die': {
        for (let layer = 0; layer < 3; layer++) {
          ...
          for (let i = 0; i < blen; i++) bdata[i] = (Math.random() * 2 - 1) * Math.pow(1 - i / blen, 0.8);
          ...
          bsrc.connect(bfilt); bfilt.connect(bgain); bgain.connect(ac.destination);
          bgain.gain.setValueAtTime(0.8, now + layer * 0.1);
          bgain.gain.exponentialRampToValueAtTime(0.001, now + 0.5 + layer * 0.15);
          bsrc.start(now + layer * 0.1);
        }
        const bosc = ac.createOscillator();
        const bogain = ac.createGain();
        bosc.connect(bogain); bogain.connect(ac.destination);
        bosc.type = 'sine';
        ...
        bosc.start(now); bosc.stop(now + 0.6);
        break;
      }

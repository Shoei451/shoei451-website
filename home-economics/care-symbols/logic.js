...
function getCorrectRate(questionId) {
  if (!stats.totalAttempts[questionId] || stats.totalAttempts[questionId] === 0)
    return null;
  return (
    (stats.correctAttempts[questionId] / stats.totalAttempts[questionId]) * 100
  );
}
  ...
  switch (mode) {
    ...
    case "unattempted":
      return questions.filter(
        (q) => (stats.totalAttempts[q.id] || 0) < MIN_ATTEMPTS,
      );
    default:
      return questions;
  }
...
document.getElementById("startBasicBtn").addEventListener("click", () => {
  ...
  if (filtered.length === 0) {
    alert(
      "このモードで出題できる問題がまだありません。ランダム練習をお試しください。",
    );
    return;
  }
  ...
  selectedQuestionCount =
    countValue === "all" ? filtered.length : parseInt(countValue);

...
(function () {
  ...
  if (!slug) {
    document.body.textContent = 'slug パラメータが指定されていません（例: ?slug=history）';
    return;
  }
  ...
  listScript.onload = () => {
    ...
    if (!cfg) {
      document.body.textContent = slug + '/list.js に PAGE_CONFIG が定義されていません。';
      return;
    }
    buildPage(cfg, slug);
  };
  ...
  function isAbsolute(s) {
    return !s || s.startsWith('http://') || s.startsWith('https://')
              || s.startsWith('/') || s.startsWith('#') || s.startsWith('mailto:');
  }
  ...
  function buildPage(cfg, slug) {
    ...
    navigator.sendBeacon(
      '/api/sw?path=' + encodeURIComponent(location.pathname + location.search) +
      '&ref='         + encodeURIComponent(document.referrer)
    );

    // ── 依存スクリプトを順次ロード → カード生成 ──────────────
    // ルート直下から参照するので js/ 固定
    const scripts = ['js/theme-toggle.js', 'js/card-generator.js', 'js/cards-page-init.js'];
    let i = 0;
    function loadNext() {
      if (i >= scripts.length) {

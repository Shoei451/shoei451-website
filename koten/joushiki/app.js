...
function buildStartCats() {
  ...
  wrap.innerHTML = "";
  [["mix", "Mix mode"], ...Object.entries(CAT_LABELS)].forEach(
    ([key, label]) => {
...
// ══════════════════════════════════════════════
function buildDeck() {
  let src = allQuestions.filter(
    (q) => selectedCategory === "mix" || q.category === selectedCategory,
  );
...
function setStartCategory(cat) {
  ...
  document
    .querySelectorAll("[data-start-cat]")
    .forEach((b) => b.classList.toggle("active", b.dataset.startCat === cat));
  updateStartMeta();
}
...
function renderCard() {
  ...
  fb.className = "face-badge badge-cat badge-" + q.category;
...
function prevCard() {
  if (!isCardTransitioning && deckIdx > 0) transitionToCard(deckIdx - 1, -1);
}
...
async function saveQuestion() {
  ...
  try {
    const [row] = await sbPost("/rest/v1/koten_questions", {
      category,
      question,
      answer,
      explanation,

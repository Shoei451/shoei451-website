...
function parseSections(html) {
  const sections = [];
  const re =
    /class="section-divider"[\s\S]*?(?=class="section-divider"|<\/body>|<nav\b|<footer\b)/g;
  let m;
  while ((m = re.exec(html)) !== null) {
    const block = m[0];
    const title =
      extract(block, /class="section-title"[^>]*>([\s\S]*?)<\/h2>/i) ?? "";
    const desc =
      extract(block, /class="section-description"[^>]*>([\s\S]*?)<\/p>/i) ?? "";
...
function parseInitCardsPage(html) {
  ...
  const entries = [];
  const re1 =
    /\{[\s\S]*?itemsVar\s*:\s*["']([^"']+)["'][\s\S]*?containerId\s*:\s*["']([^"']+)["'][\s\S]*?\}/g;
  let em;
  while ((em = re1.exec(inner)) !== null)
    entries.push({ itemsVar: em[1], containerId: em[2] });
  if (entries.length) return entries;
  const re2 =
    /\{[\s\S]*?containerId\s*:\s*["']([^"']+)["'][\s\S]*?itemsVar\s*:\s*["']([^"']+)["'][\s\S]*?\}/g;
  while ((em = re2.exec(inner)) !== null)
...
function buildPageConfig(html, folderName) {
  ...
  const sections = sectionDefs.map((s, i) => {
    const map = mappings.find((m) => m.containerId === containerIds[i]) ??
      mappings[i] ?? {
        itemsVar: `sectionItems${i + 1}`,
        containerId: containerIds[i] ?? `c${i + 1}`,
      };
    return {
      title: s.title,
      desc: s.desc,
      itemsVar: map.itemsVar,
      id: map.containerId,
    };
  });

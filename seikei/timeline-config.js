// seikei/timeline-config.js
// timeline.html?slug=seikei 用設定
...
window.TIMELINE_CONFIG = {
  ...
  categories: [
    ...
    { id: '消費者生活・公害対策', label: '消費者・公害', bg: '#e0f2fe', fg: '#0c4a6e' },
  ...
  async fetchData() {
    const { data, error } = await window._db
      .from('seikei_events')
      .select('id, year, year_end, date_type, full_date, title, description, category')
  ...
  getEvent(row)       { return row.title; },
  getDescription(row) { return row.description; },
  getCategory(row)    { return row.category; },
};
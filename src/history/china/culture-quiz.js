...
function startQuiz(mode) {
  resetState();
  state.mode = mode;
  if (mode === "person-to-dynasty") {
    genP2D();
    renderP2D();
    showView("P2D");
  } else if (mode === "dynasty-to-people") {
    genD2P();
    renderD2P();
    showView("D2P");
...
function genP2D() {
  state.questions = shuffle(quizData)
    .slice(0, 10)
    .map((p) => {
      const wrong = shuffle(dynasties.filter((d) => d !== p.dynasty)).slice(
        0,
        3,
      );
      return {
        question: p.name,
        options: shuffle([...wrong, p.dynasty]),
        correct: p.dynasty,
        info: p.category,
        type: "single",
      };
...
function nextP2D() {
  if (state.currentQ < state.questions.length - 1) {
    ...
    showResults(state.score, state.questions.length);
  }
}
...
function genD2P() {
  ...
  state.questions = shuffle(avail)
    .slice(0, 5)
    .map((dynasty) => {
      const correct = quizData.filter((p) => p.dynasty === dynasty);
      const wrong = shuffle(
        quizData.filter((p) => p.dynasty !== dynasty),
      ).slice(0, 4);
      return {
        question: dynasty,
        options: shuffle([...correct, ...wrong]).map((p) => p.name),
        correct: correct.map((p) => p.name),
        type: "multiple",
      };
    });
}
...
function renderD2P() {
  ...
  grid.innerHTML = q.options
    .map((name) => {
      ...
      const onClick = state.showResult
        ? ""
        : `onclick="toggleD2P('${escHtml(name)}')"`;
      // 変更後
      const personDynasty =
        quizData.find((p) => p.name === name)?.dynasty ?? "";
      return `
...
// ════════════════════════════════
function genMatch() {
  const picked = shuffle(quizData.filter((p) => p.works.length > 0)).slice(
    0,
    6,
  );
  state.matchData = {
    people: picked.map((p, i) => ({
      id: `person-${i}`,
      text: p.name,
      type: "person",
    })),
...
function renderMatch() {
  ...
  document.getElementById("match-works").innerHTML = d.works
    .map((item) => {
...
function handleMatch(item) {
  ...
  if (!state.selectedMatch) {
    ...
    if (state.selectedMatch.type !== item.type) {
      ...
      const isCorrect = d.correct.some(
...
function genTF() {
  ...
  quizData.forEach((p) => {
    dyStmts.push({
      statement: `${p.name}は${p.dynasty}の人物である`,
      correct: true,
    });
    const wrong = shuffle(dynasties.filter((d) => d !== p.dynasty))[0];
    dyStmts.push({
      statement: `${p.name}は${wrong}の人物である`,
      correct: false,
    });
  });
  ...
  withWorks.forEach((p) => {
    wkStmts.push({
      statement: `${p.name}の主な功績に『${p.works[0]}』がある`,
      correct: true,
    });
    const other = shuffle(withWorks.filter((q) => q.name !== p.name))[0];
    if (other)
      wkStmts.push({
        statement: `${p.name}の主な功績に『${other.works[0]}』がある`,
        correct: false,
      });
  });
...
function renderTF() {
  ...
  if (state.showResult) {
    if (q.correct === true) {
      tCls += " correct";
      if (falseSelected) fCls += " incorrect";
    } else {
      fCls += " correct";
      if (trueSelected) tCls += " incorrect";
    }
  } else {
...
function genTimeline() {
  ...
  dynasties.forEach((d) => {
    ...
    if (cands.length)
      repMap.set(d, cands[Math.floor(Math.random() * cands.length)]);
  });
  const picked = shuffle([...repMap.values()]).slice(0, 6);
  state.tlCorrect = [...picked]
...
function renderTimeline() {
  ...
  document.getElementById("tl-list").innerHTML = items
    .map(
      (item, idx) => `
    <div class="sort-item">
...
function submitTimeline() {
  ...
  userOrder.forEach((name, idx) => {
    if (name === correctOrder[idx]) correct++;
  });
  state.score = correct;

  // 結果を視覚化してからResultsへ
  const list = document.getElementById("tl-list");
  list.innerHTML = state.tlItems
    .map((item, idx) => {
      const isCorrect = item.name === correctOrder[idx];
      const cls =
        "sort-item " + (isCorrect ? "result-correct" : "result-wrong");

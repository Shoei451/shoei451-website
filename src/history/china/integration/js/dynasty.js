...
function parseClozeHint(mdText) {
  ...
  html = html.replace(
    /CLZPH_(\d+)_END/g,
    (_, i) => `<span class="cloze-hint">${escHtml(map[+i])}</span>`,
  );
  return html;
}
...
function renderTimeline(events) {
  ...
  if (!events.length) {
    wrap.innerHTML =
      '<div class="empty-state"><span>📜</span>この王朝の年表データがありません。</div>';
    return;
  }
...
function renderFigures(figures) {
  ...
  if (!figures.length) {
    wrap.innerHTML =
      '<div class="empty-state"><span>👤</span>この王朝の文化人物データがありません。</div>';
    return;
  }
  const groups = {};
  figures.forEach((f) => {
...
async function init() {
  ...
  const figures = (typeof quizData !== "undefined" ? quizData : []).filter(
    (q) => dynasty.quizMatch?.includes(q.dynasty),
  );

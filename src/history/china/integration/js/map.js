    ...
    function buildTimelineHtml(d) {
      ...
      const axisHtml = `
        <div class="tl-axis" style="grid-template-columns:${
          ticks.map((y, i) => i === 0 ? '0fr' : `${(pct(y) - pct(ticks[i-1]))}fr`).join(' ')
        }">
          ${ticks.map(y => `<div class="tl-axis-tick">${y < 0 ? '前'+Math.abs(y) : y}</div>`).join('')}
        </div>`;

      const groupsHtml = tl.groups.map(g => {
        const labelHtml = g.label ? `<div class="tl-group-label">${esc(g.label)}</div>` : '';
        const rowsHtml  = g.tracks.map(t => {
          const l = pct(t.start);
    ...
    function renderCanvas() {
      ...
      canvas.innerHTML = DYNASTIES.map(d => {
        ...
        const nodeClass = [selected ? 'highlight' : '', multi ? 'multi' : '', manyClass]
          .filter(Boolean).join(' ');

        const body = multi
          ? `<div class="split-wrap">
               <div class="segments">${
                 d.territories.map(t => `<div class="seg" style="flex:${t.ratio} 1 0"></div>`).join('')
               }</div>
    ...
    function renderDetail() {
      ...
      if (!d) {
        if (isMobile) { panel.classList.remove('open'); overlay.classList.remove('open'); }
      ...
      root.innerHTML = `
        ...
        ${d.figures?.length  ? `<div class="card"><h4>主要人物</h4>
          <div class="chips">${d.figures.map(f => `<span class="chip">${esc(f)}</span>`).join('')}</div>
          </div>` : ''}
        ${d.culture ? `<div class="card"><h4>文化</h4>
    ...
    window.addEventListener('resize', () => {
      if (!state.selectedSlug) { renderDetail(); return; }
      const overlay = document.getElementById('mobileOverlay');
...
function _renderSearchResults(q, results) {
  ...
  if (!results.length) {
    root.innerHTML =
      `<div class="empty-detail">「${_esc(q)}」に一致する王朝が見つかりませんでした</div>`;
    return;
  }

  const rows = results.map(({ dynasty: d, matches }) => `

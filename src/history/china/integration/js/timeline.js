  ...
  function render() {
    ...
    if (events.length === 0) {
      container.innerHTML = '<div class="empty-state">条件に一致するイベントがありません。</div>';
      return;
    }
  ...
  function renderItem(ev) {
    ...
    if (currentQuery) {
      ...
      title = title.replace(re, '<mark style="background:var(--color-accent-light);color:var(--color-accent)">$1</mark>');
    }

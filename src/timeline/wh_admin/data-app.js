// ============================================================
// 世界史年表 管理画面 (wh_dates) - data-app.js
// ============================================================
...
function applyFilters(query) {
  if (currentRecordType !== "all")
    query = query.eq("record_type", currentRecordType);
  if (currentRegion !== "all")
    query = query.contains("region", [currentRegion]);
                    ...
                    <tr>
                        ...
                        <td>${
                          row.field
                            ...
                            : '<span style="color:var(--text-light-secondary);font-size:0.85rem;">-</span>'
                        }</td>
                        <td class="col-region">
                            ${(row.region ?? [])
                              .map(
...
function setupEventListeners() {
  ...
  document.querySelectorAll(".tab[data-record-type]").forEach((tab) => {
    tab.addEventListener("click", (e) => {
      document
        .querySelectorAll(".tab[data-record-type]")
...
function wizUpdateStep2ForType(type) {
  ...
  document.getElementById("s2-year-block").style.display =
    isEvent || isPeriod ? "" : "none";
  document.getElementById("s2-person-block").style.display = isPerson
    ? ""
    : "none";
  document.getElementById("s2-yearend-wrap").style.display = isPeriod
    ? ""
    : "none";

  document.getElementById("s2-year-label").textContent = isPeriod
    ? "開始年"
    : "年";
  document.getElementById("s2-event-label").textContent = isPerson
    ? "人物名"
    : "出来事";
...
function wizInitStep2() {
  ...
  document
    ...
    .addEventListener("change", wizCheckDuplicate);
}
...
function wizBuildConfirm() {
  ...
  const typeLabel =
    { event: "出来事", period: "期間", person: "人物" }[type] ?? type;
  ...
  const rows = [
    ...
    [
      type === "person" ? "人物名" : "出来事",
      event || '<span class="confirm-empty">（未入力）</span>',
    ],
    ["地域", regionLabels],
    ...
    [
      "Wikipedia",
      wikiUrl
        ? `<a href="${escapeHtml(wikiUrl)}" target="_blank" style="color:var(--accent-gold);">リンク</a>`
        : '<span class="confirm-empty">なし</span>',
    ],
...
function wizValidateStep(step) {
  ...
  if (step === 2) {
    ...
    if (!event) {
      alert(
        `${WIZ.selectedType === "person" ? "人物名" : "出来事"}を入力してください`,
      );
